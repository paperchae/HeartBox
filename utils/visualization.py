...
def train_validation_dataset_distribution(
    ...
    * To Ensure that the train and validation dataset have similar distribution
    ...
    plt.title(
        "Train/Validation Dataset Distribution"
        + "\n\n"
        + "Train True: "
        + str(train_true)
        + "% Validation True: "
...
def plot_training_results(
    train_loss: list, valid_loss: list, test_loss: list, saved_epochs: list
) -> None:
    """
    main.py
...
def hist_inference_data_by_age() -> None:
    ...
    external_df = pd.read_csv(
        "data/index/external/external_test_index_temp.csv", index_col=0
    )
    ...
    internal_train_list = internal_train_df[internal_train_df["AFIB_OR_AFL"] == 1][
        "AGE"
    ].to_numpy()
    internal_test_df = pd.read_csv("data/index/internal/test_index.csv", index_col=0)
    internal_test_list = internal_test_df[internal_test_df["AFIB_OR_AFL"] == 1][
        "AGE"
    ].to_numpy()
...
def plot_sensitivity_verse_specificity(prob: list, label: list) -> None:
    ...
    for threshold in threshold_list:
        sensitivity, specificity = Metrics(
            prob, label, threshold
        ).sens_and_spec_with_cutoff()
