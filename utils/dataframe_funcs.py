    ...
    for h in height_list:
        ...
        if target == 'Height':
            ...
            if test['VALUEUOM'].tolist()[0] in ['lb', 'pound', 'pounds', 'lbs']:
                test.loc[test['VALUEUOM'].str.contains('lb|pound|pounds|lbs'), 'VALUE'] = uom_converter(test['VALUE'],
                                                                                                        target)
...
def read_chartevent(subject_list, debug=False):
    ...
    # aortic_sys_list = []
    # aortic_dia_list = []
    for chunk in pd.read_csv(raw_data_path + 'CHARTEVENTS.csv.gz', chunksize=chunk_size, compression='gzip', header=0,
    ...
    total_df = reduce(lambda x, y: pd.merge(x, y, on='SUBJECT_ID', how='outer'), total_list)
...
def read_patients(subject_list, debug=False):
    ...
    merged_info_df = pd.merge(patient_df, admission_df, how='outer', on='SUBJECT_ID').drop_duplicates(['SUBJECT_ID'],
                                                                                                      keep='last')
    ...
    merged_df = pd.merge(merged_info_df, health_df, how='outer', on='SUBJECT_ID')[['SUBJECT_ID', 'GENDER', 'AGE',
