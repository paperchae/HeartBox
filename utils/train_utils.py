...
def guarantee_reproducibility(random_seed: int = 125) -> Any:
    ...
    if torch.cuda.is_available():
        ...
        cudnn.enabled = True
        cudnn.deterministic = (
            True  # turn on for reproducibility ( if turned on, slow down training )
        )
...
def check_preprocessed_data(cfg) -> None:
    ...
    """
    data_list = [
        "internal/test_normal.hdf5",
        "internal/test_augmented_af.hdf5",
        "internal/train_normal.hdf5",
        "internal/train_augmented_af.hdf5",
        "external/test.hdf5",
    ]
