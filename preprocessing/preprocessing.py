...
def save_preprocessed_data(cfg,
                           preprocessed_ecg,
                           file_name,
                           age) -> None:
    """
    ...
    if cfg.debug:
        ...
        dset['label'] = np.ones_like(file_name, dtype=float) \
            if cfg.preprocess.data.af else np.zeros_like(file_name, dtype=float)
        dset['age'] = age
        dset.close()
...
def read_total_data(cfg: Any) -> None:
    ...
    preprocessed_ecg, file_name, _, age = multi_process(target_function=read_waveform_data,
                                                        patient_df=idx_df,
...
if __name__ == "__main__":
    ...
    if config.preprocess.all:
        ...
        sweep = [[True, True, False, False],  # internal, train, normal, no augment
                 ...
                 [False, None, None, False]]  # external, None, None, no augment
        for s in sweep:
            print(s)
